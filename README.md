...
1. **Add the config file.** `vercel.json` defines a Python function at `api/run.py` and a daily cron (9:00 UTC by default). Vercel Hobby plans only allow **one run per day**; to run more frequently, upgrade your plan and adjust the schedule accordingly.
