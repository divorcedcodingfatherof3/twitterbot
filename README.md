  ...
  ping:
    ...
    steps:
      - name: Trigger bot endpoint
        run: |
          curl -sS https://<your-vercel-project>.vercel.app/api/check-tweets
```

Replace `<your-vercel-project>` with your deployment hostname. Keep secrets in Vercel; the workflow only needs network access to the public endpoint.
